...
6. Isuuses:
   - I want to improve my Right side bar and left side bar in whole project same as in `feature-specs/image/image.png`
   - bug : the zoom controller is overriding the project's side bar make it hide behing the project side bar when side bar is opened.
   - ui fix : at `/editor` tab redesign TOP-BAR ui as same as in the image.
